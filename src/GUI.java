...
import org.eclipse.wb.swt.TwoPartAlgorithm;
//========================================================================Imports=====================================================================================================================
//========================================================================Initializations=============================================================================================================
public class GUI {
	...
	public class OutputPane 
	{
		...
		{
			output = new JFrame();
			JOptionPane.showMessageDialog(output,"Steal Sequence is " + outSignal);
		}
	}
	...
	public class StatPane 
	{
		...
		{
			output = new JFrame();
			JOptionPane.showMessageDialog(output,"Accuracy: " + accuracy + "% Precision: " + precision + "% Recall: " + recall + "% F1 Score: " + F1 + "%");
		}
	}
...
//================================================================================GUI Runs and Initializes Inputs===================================================================================
	public void open() {
	...
	protected void createContents() {
		...
		shlrdBaseCoach.setSize(1000, 598);
		...
		Button testButton = new Button(shlrdBaseCoach, SWT.NONE);
		testButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseDown(MouseEvent e) {
				...
				{
					String tempStr;
					File DataFile = new File("AllData.txt");
					try {
						Scanner ScFile = new Scanner(DataFile);
						while(ScFile.hasNextLine())
						{
							tempStr = ScFile.nextLine();
							
							if (outSignal.contains(".")) {
	                            String start = String.valueOf(outSignal.charAt(0));
	                            String end = String.valueOf(outSignal.charAt(2));
							...
							if (tempStr.contains(outSignal) == true)
							{
								if (tempStr.contains("1") == true)
									TP += 1;
								else
		...
		testButton.setBounds(10, 488, 198, 25);
		formToolkit.adapt(testButton, true, true);
		testButton.setText("Test Prediction Accuracy");
		
		//================================================================Begin Scrolled Component===============================================================================================
		...
		//=========================================================Initialize all 26 ComboBoxes with their allowed inputs=======================================================================
		...
		//==========================================================================================Initialize Description Text Boxes=======================================================
		...
		File dataFile = new File ("AllData.txt");
		//text.setText(writeLog(dataFile,0));
		
		Button btnPredict = new Button(shlrdBaseCoach, SWT.NONE); //========================================Predict Button===================================================================
		btnPredict.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseDown(MouseEvent e) {
				//Predict Here
				TwoPartAlgorithm predictor = new TwoPartAlgorithm();
				outSignal = predictor.main();
				//output.setText("Steal Sequence is " + predictor.main());
				//Output
				new OutputPane();
				prediction = 1;
			}
		});
		btnPredict.setBounds(10, 457, 75, 25);
		formToolkit.adapt(btnPredict, true, true);
		...
		stealBTN.addMouseListener(new MouseAdapter() { //========================================Steal Button===================================================================
			@Override
			public void mouseDown(MouseEvent e) {
				String newLog = "";
				String stealLine = "";
				String input = seqInput.getText();
				//Separate each individual letter by spaces and condense it into the desired file format
				...
				try {
					...
					if (!input.trim().isBlank())//Making sure that the user entered a sequence
					{
					FileWriter writer = new FileWriter("AllData.txt");
					writer.write(stealLine.toString());
					writer.close();
					newLog = writeLog(dataFile,1);
					}
				} catch (IOException e1) {
					e1.printStackTrace();
				} finally {
		...
		btnNoSteal = new Button(shlrdBaseCoach, SWT.NONE);
		btnNoSteal.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseDown(MouseEvent e) { //========================================No Steal Button===================================================================
				String newLog = "";
				String stealLine = "";
				String input = seqInput.getText();
				//Separate each individual letter by spaces and condense it into the desired file format
		...
		btnNoSteal.setBounds(91, 426, 75, 25);
		formToolkit.adapt(btnNoSteal, true, true);
		btnNoSteal.setText("No Steal");
		
		clearLog = new Button(shlrdBaseCoach, SWT.NONE);
		clearLog.addMouseListener(new MouseAdapter() { 
			@Override
			public void mouseDown(MouseEvent e) { //================================================Clear Log===============================================================
				FileWriter writer;
				try {
					writer = new FileWriter("AllData.txt");
					writer.close();
				} catch (IOException e1) {
					e1.printStackTrace();
				}
				log = ""; //Reset Log
				text.setText(""); //Reset text Output
				prediction = 0;
			}
		});
		clearLog.setBounds(899, 457, 75, 25);
		formToolkit.adapt(clearLog, true, true);
		clearLog.setText("Clear Log");
		
		//==================================================================Checks for Var changes and sees if it is a duplicate value=====================================
	...
	//============================================================================Check for duplicate values in var list===============================================================
	...
	//=======================================================================Writes to the log============================================================================
	protected String writeLog(File data, int times)
	{
		String newLog = "";
		int index;
		String line = "";
		...
		for (int i = 0; i < sequences.length-2; i ++)
		{
			...
			index += 1;
			if (index != 0) //=========================================================================Displays description after Var=============================================
			{
				...
				if (seq26.getSelectionIndex() == index) { newLog += desc26.getText() + " "; }
